...
namespace EmulatorEngine
{
    public class CDumpData : CDataConversion
    {
        public void DisassembleData (byte[] yaMainMemory)
        {
            if (iStart  > 0 &&
                iLength > 0)
            {
                PrintStringList (DisassembleCodeImage (yaMainMemory, iStart, iStart + iLength));
            }
    ...
    public partial class CDataConversion : CStringProcessor
    {
        ...
        public List<string> DisassembleIplLine (string strIplLine)
        {
            ...
            {
                strlIplLines.AddRange (DisassembleCodeImage (ReadIPLCard (strIplLine), 0x0000, 0x005F, 0x0000));
            }
        ...
        public List<string> DisassembleTextFile (string strFilePath)
        {
            PrepForDASM ();

            // Load file to byte array
            LoadTextFile (strFilePath);
            m_bDumpComplete = false;

            // Disassemble end card
            List<string> strlDisassembleTextFile = new List<string> ();
            int iLastSlash = strFilePath.LastIndexOf ('\\');
            if (iLastSlash > 0 &&
                iLastSlash < strFilePath.Length)
            {
                strlDisassembleTextFile.Add ("Program Disassembly for: " + strFilePath.Substring (iLastSlash + 1));
            }
            else
            {
                strlDisassembleTextFile.Add ("Program Disassembly for: " + strFilePath);
            }
        ...
        public List<string> DisassembleTextFile (List<string> lstrCardData, string strItemName)
        {
            PrepForDASM ();
        ...
        public byte[] LoadTextFile (string strFilename)
        {
            // Load file to string list
            List<string> strlTextFile = ReadFileToStringList (strFilename, 96);
        ...
        protected int m_iLowAddressMI  = 0x0000;
        protected int m_iHighAddressMI = 0x0000;
        protected int m_iEntryPointMI  = 0x0000;
        ...
        protected int m_iEndCardLoadAddress = 0;
...
//                                  08    09    8A    8B    8C    8D    8E    8F
        ...
        static public byte[] ConvertUShortToByteArray (ushort usData)
        {
            byte[] yaData = new byte[4];

            for (int iIdx = 4; iIdx-- > 0;)
            {
                yaData[iIdx] = (byte)(0x10 + (usData & 0x000F));
                usData >>= 4;
            }
        ...
        static public ushort ConvertByteArrayToUShort (byte[] yaData)
        {
            ushort usData = 0;
            int iArrayIdx = yaData.GetUpperBound (0);
            ...
            for (int iIdx = 0; iIdx <= iArrayIdx; ++iIdx)
            {
                usData <<= 4;
                usData += (ushort)(yaData[iIdx] & 0x000F);
            }
