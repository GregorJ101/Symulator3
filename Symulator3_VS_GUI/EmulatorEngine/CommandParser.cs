...
namespace EmulatorEngine
{
    ...
    public partial class CDataConversion : CStringProcessor
    {
        ...
        public bool ProcessCardCommandLine (string strCommandLine)
        {
                     ...
                     CompareNoCase (strlTokens[0], "DASM-Range"))
            {
                ...
                if (CompareNoCase (strlTokens[0], "DASM"))
                {
                    ...
                    {
                        PrintStringList (DisassembleCodeImage (m_yaProgramImage, m_iDasmStart, m_iDasmEnd));
                    }
                    else if (m_iLowAddressMI  < m_iHighAddressMI &&
                             m_iHighAddressMI > 0)
                    {
                        PrintStringList (DisassembleCodeImage (m_yaProgramImage, m_iLowAddressMI, m_iHighAddressMI));
                    }
                    else
                    {
                        PrintStringList (DisassembleCodeImage (m_yaProgramImage));
                    }
                }
            ...
            else if (CompareNoCase (strlTokens[0], "DASM-EndCard"))
            {
                    ...
                    m_iDasmEndEC   > m_iDasmStartEC)
                {
                    ...
                    PrintStringList (DisassembleCodeImage (m_yaEndCard, m_iDasmStartEC, m_iDasmEndEC));
                    RestoreListDCP ();
                }
                         ...
                         m_iHighAddressEC > 0)
                {
                    ...
                    PrintStringList (DisassembleCodeImage (m_yaEndCard, m_iLowAddressEC, m_iHighAddressEC));
                    RestoreListDCP ();
                }
                else
                {
                    BackupAndClearListDCP (0x019, m_yaEndCard.GetUpperBound (0));
                    PrintStringList (DisassembleCodeImage (m_yaEndCard));
                    RestoreListDCP ();
                }
        ...
        private void BackupAndClearListDCP (int iLowAddress, int iHighAddress)
        {
            ...
            m_sdDCP[iLowAddress]  = new CDasmControlPoint (iLowAddress,  0, 0, true, EControlPointType.POINT_Begin, "Low address");
            m_sdDCP[iHighAddress] = new CDasmControlPoint (iHighAddress, 0, 0, true, EControlPointType.POINT_End,   "High address");
        ...
        int InsertDasmControlPoint (List<string> strlTokens, ENotationType eNotationType)
        {
            ...
            return 0; //  InsertDasmControlPoint (iAddress, 0, 0, true, eNotationType, strNotation);
        }
