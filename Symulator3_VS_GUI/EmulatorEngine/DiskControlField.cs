...
{
    public class CDiskControlField : CControlFlags
    {
        ...
        public string DisplayStates (bool bFromSIO)
        {
            if (bFromSIO)
            {
                if (bStateSaved)
            ...
            {
                return string.Format ("            DCF after LIO -  F:{0:X2} C:{1:X2} S:{2:X2} N:{3:X2}", yF, yC, yS, yN);
            }
